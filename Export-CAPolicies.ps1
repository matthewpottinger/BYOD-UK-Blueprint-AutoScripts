...
$clientId = "bcc52518-f860-4fd0-ab37-98bb09666665"
...
Returns any conditional access policies configured in the organization
.NOTES
...
$CAPolicies = Get-ConditionalAccessPolicy

foreach($CAPolicy in $CAPolicies){

    Write-Host "CA Policy:"$CAPolicy.displayName -f Yellow
    Export-JSONData -JSON $CAPolicy -ExportPath "$ExportPath"
    Write-Host
